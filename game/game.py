...
class Game:
    ...
    def attack(self, player, attacker, target):
        ...
        # Check target health safely
        is_target_alive = getattr(target, "is_alive", target.health > 0)
        if not is_target_alive:
            raise ValueError("Target is already defeated")

        # Range check
        attack_range = getattr(attacker, "attack_range", 1)
        if self._distance(attacker.position, target.position) > attack_range:
            raise ValueError("Target is outside the attack range")

        # --- COMBAT RESOLUTION ---
        # Determine damage using attacker's power
        damage = getattr(attacker, "attack_power", 10)

        # Apply damage to target safely
        if hasattr(attacker, "attack") and callable(attacker.attack):
            damage = attacker.attack(target) or damage
        elif hasattr(target, "take_damage") and callable(target.take_damage):
            target.take_damage(damage)
        else:
            target.health -= damage

        # Clean up defeated target
        target_still_alive = getattr(target, "is_alive", target.health > 0)
        if not target_still_alive:
            if hasattr(target_owner, "remove_unit"):
                target_owner.remove_unit(target)
            elif target in target_owner.units:
                target_owner.units.remove(target)

            if hasattr(self._map, "remove_unit"):
